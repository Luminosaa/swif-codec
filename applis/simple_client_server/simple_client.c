 ...
 */
static void decoded_source_symbol_callback(void *context, void *new_symbol_buf, esi_t esi);

void **src_symbols_tab =
    NULL; /* table containing pointers to the source symbol buffers, either received from the
...
src_symbol_status_at_recv_t *src_symbols_status_tab =
    NULL; /* table containing source symbol status: missing/received/decoded. This is
...
void **repair_symbols_tab =
    NULL; /* table containing pointers to the received repair symbol buffers.
...
{
    bool *src_symbols_allocated = NULL;
    ...
    /* First of all, initialize the UDP socket and wait for the FEC OTI to be received. This is
     * absolutely required to synchronize encoder and decoder. We assume this first packet is NEVER
     * lost otherwise decoding is not possible. In practice the sender can transmit it periodically,
     * or it is sent through a separate reliable channel. */
    ...
    {
        fprintf(stderr,
                "Error, FEC OTI reception failed: bad size, expected %lu but received %d instead\n",
    ...
    printf(
        "FEC_OTI: Receiving packets from %s/%d: codepoint %u, ew_size=%u, tot_src=%u, tot_enc=%u\n",
    ...
    {
        fprintf(stderr,
                "Error, invalid FEC OTI received: tot_src_symbols=%u or tot_encoding_symbols=%u "
                "received are probably out of range\n",
                tot_src, tot_enc);
        ret = -1;
        goto end;
    }
    ...
    if(((src_symbols_tab = (void **)calloc(tot_src, sizeof(void *))) == NULL) ||
       ((src_symbols_status_tab = (src_symbol_status_at_recv_t *)calloc(tot_src, sizeof(void *))) ==
        NULL) ||
       ((repair_symbols_tab = (void **)calloc(tot_enc - tot_src, sizeof(void *))) == NULL) ||
       ((src_symbols_allocated = (bool *)calloc(tot_src, sizeof(bool))) == NULL))
    {
        fprintf(stderr, "Error, no memory (tot_enc=%u)\n", tot_enc);
        ret = -1;
        goto end;
    }
    /* Open and initialize the decoding session now that we know the various parameters used by the
     * sender/encoder... */
    ...
    if(swif_decoder_set_callback_functions(ses, NULL, NULL, &decoded_source_symbol_callback,
                                           NULL) != SWIF_STATUS_OK)
    {
    ...
    while((ret = get_next_pkt(so, (void **)&pkt, &len)) == SWIF_STATUS_OK)
    {
        ...
        else if(is_repair_packet(len))
        {
            ...
            if(swif_decoder_decode_with_new_repair_symbol(ses, repair_symbols_tab[rep_idx], esi) !=
               SWIF_STATUS_OK)
            {
    ...
    if(src_symbols_tab && src_symbols_status_tab && repair_symbols_tab && src_symbols_allocated)
    {
        for(esi = 0; esi < tot_src; esi++)
        {
            if(src_symbols_allocated[esi])
            {
                if(src_symbols_status_tab[esi] == SRC_SYMBOL_STATUS_RECEIVED)
                {
                ...
                src_symbols_allocated[esi] = false; /* Mark as freed */
            }
        }
        free(src_symbols_tab);
        free(src_symbols_status_tab);
        free(src_symbols_allocated);
